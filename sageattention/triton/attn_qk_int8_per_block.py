...
def _attn_fwd_inner(acc, l_i, m_i, q, q_scale, qo_len, kv_len,
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        if not skip:
            ...
            if HAS_DELTA_S:
                ds = tl.load(delta_s_ptrs, mask=offs_n < (kv_len - start_n), other=0.0)
                qk += ds[None, :]
        ...
        if HAS_DELTA_S:
            delta_s_ptrs += BLOCK_N
    return acc, l_i, m_i

@triton.jit
def _attn_fwd(Q, K, V, Q_scale, K_scale, Out, mask, Lse,
              Delta_s, stride_dsz, stride_dsh, stride_dsm,
    ...
    if HAS_DELTA_S:
        delta_s_ptrs = Delta_s + (off_z * stride_dsz + off_h * stride_dsh + start_m * stride_dsm) + offs_n
    else:
        delta_s_ptrs = None
...
def forward(q, k, v, q_scale, k_scale, tensor_layout="HND", attn_mask=None, output_dtype=torch.float16, return_lse=False, delta_s=None):
    BLOCK_M = 128
    BLOCK_N = 64
    stage = 1
    ...
    if delta_s is not None:
        stride_ds_z, stride_ds_h, stride_ds_m = delta_s.stride(0), delta_s.stride(1), delta_s.stride(2)
        HAS_DELTA_S = True
    else:
        delta_s = torch.empty(0, device=q.device, dtype=torch.float32)
        stride_ds_z, stride_ds_h, stride_ds_m = 0, 0, 0
        HAS_DELTA_S = False
    ...
    _attn_fwd[grid](
        ...
        delta_s, stride_ds_z, stride_ds_h, stride_ds_m,
        ...
        STAGE=stage, RETURN_LSE=return_lse, HAS_DELTA_S=HAS_DELTA_S,
